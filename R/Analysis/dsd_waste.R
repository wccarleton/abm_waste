...
for(k in dirs){
   ...
   for(j in dataobjects){
      ...
      Z_neutral <- matrix(agent_waste_fitness[cohort,"neutral"],ncol=1)
      ...
      dsd_waste <- analyze.selection(Z_waste,W)[[1]]
      dsd_neutral <- analyze.selection(Z_neutral,W)[[1]]

      dsd_waste_sample <- c(dsd_waste_sample,dsd_waste)
      dsd_neutral_sample <- c(dsd_neutral_sample,dsd_neutral)
   }
   save(dsd_waste_sample,file=paste("../Results/DSD/","dsd_waste_sample_",k,".RData",sep=""))
